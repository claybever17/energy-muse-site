...
Sara (review, 1:36) asked whether the reviews should sit higher on the page, without saying where, and offered more than three. Clay chose the slot: `#stories` now sits under the frequency section and before "Not sure where to begin?", up from second-to-last, so the quiz reads as the next step after real people's stories and the product grid and the frequency story stay together. Still three quotes; she will send more. The homepage also loaded `em-bag.js?v=10` while every other page was at v11; it is v11 now.
