...
### Sept 9: the journal index earns its kicker
Clay: "journal landing is bland? no? seems off?" It was a photo, a title and a cut excerpt per card: their articles came over with no date and no author, so the small line under every card printed nothing, and nothing on the page connected the words to the pieces. Now: the month's post keeps its hero and carries "Shop this post" under it, the same four products the article names by the same matcher (the fallback is one of each kind, so the index always has a rail - Sara, 14:15, wanted a feed on the blog); every card's kicker is the stones the post works with, the matched base names cased and in copper ("Rose Quartz · Selenite"), or nothing when a post names no piece; the hero says "Read the post"; eight posts sit as two rows of four on a wide screen instead of three rows with two orphans. The matcher and the product card moved out of the article branch, where strict mode had block-scoped them, so the index can use them; the article view is unchanged. Dates and authors join the kicker on their own on Shopify.

### Sept 9: a video slot on every generator page
Sara, by email: "I am shooting videos with Heather today for each of the Frequency devices." The meaning pages already had the slot; the generator product pages have the same one now, "Heather on 963 Hz", between the panels and the placement band: with `video` in the generator's record in em-generators.js ({ youtube: id } or { vimeo: id } or { src }) it is a poster and a play button and the player loads on the click; without one it is the tinted frame and "Video coming". `?video=yt:ID` or `?video=vimeo:ID` previews the player on any generator without touching the data. When her films arrive: one line per generator in em-generators.js, then bump its version on the product page.

### Sept 9: the generator prices are live
Sara, by email to Eric: "Generator pricing is $99.88." em-prices.js MODE is 'live' (v4 everywhere); every generator shows $99.88 and the room cards on the Formulas page show the real sum, $299.64. The hold ('$—') can come back by setting MODE to 'hold' or per visit with ?prices=hold. In the same email: she is shooting a video per frequency device today and may find older crystal videos for the meaning pages; she will make all seven room Formulas products within days, at which point the five three-generator cards become kit cards from her store; she will review the pages she skipped; she asks who to give admin access to (the collaborator request from Clay's Partners account); and she will record per-page Looms rather than one long one.

### Sept 9: one page, one name, one place; the meanings index; the stone dropdown
Three from the sweep, all Clay's call ("do all 3"). **The nav and both footers.** Clay, on the Learn menu: "they all go to the same spot." The Learning Center is one page with three fold-out sections and had been listed as three sections, as All guides and as Beginner's Guide: five nav entries for one URL, and three footer entries. The FAQ was in the footer twice under two names. Rule now: each page once, one name, one place. Nav: Start Here is Energy Quiz and How it works; Learn is Learning Center, Crystal Meanings, Journal. Footer (em-header's `FOOT` and the homepage's static markup, kept in step): Start Here is Energy Quiz, How it works, By Intention, Make it yours; Shop says Frequency, as the nav does; Learn is Learning Center, Crystal Meanings, Journal, With Heather, Veza; Help is Contact, Shipping & Returns, Our Story, Affiliates, Materials & Craft (up from the legal line, which keeps Accessibility). em-header v51 everywhere. **Crystal Meanings has a landing.** `/meaning/` with no `?stone=` showed Black Tourmaline; it is an index now, one card per stone with a guide (nine), the c365 photo, the name, and what it is for from the first two of its own uses. Three across, two on a phone. A stone page is unchanged. **Shop by stone is a dropdown.** The crystals page stacked three filter rows in three styles and the stone row wrapped to two lines; the stone filter is a select beside Sort now, same counts, one row less, still `?stone=` in the URL and still composing with the intention and the type. The back button now restores the stone too (it had been dropped from the popstate repaint).

### Sept 11: the meaning pages carry their whole page
Sara (meaning Loom, 0:30): "make sure that all of the content that is on the current page is carried over". Ours carried about 12% of their words (Heather had asked for "much simpler"). Clay: "we need to obey the 9/10 loom". Both now: the short page stays as it was, and under How to use it a section called The full guide folds in every section of the stone's live energymuse.com page, word for word, under their own headings (You Need Black Tourmaline If..., How to Use Your Black Tourmaline Crystal for Protection, History & Lore, Origin & Regionality...). Pulled into `assets/copy/meaning/<stone>.json` (9 stones, 11,800 words; paragraphs and lists kept, links unwrapped, pictures dropped, repeated blocks merged); the page fetches it and, once it is there, hides the small History-and-origin fold it replaces. Folded text is in the page for SEO, the way the collection descriptions are. Black Tourmaline reads 1,844 words now, from 299.

### Sept 11: the second pair of eyes, and what it caught
Clay: "I don't want to tell Sara I updated the site if we missed anything. Please double check." An independent reviewer (a fresh subagent given only the eleven transcripts and the live URL, told to assume nothing was done) extracted 89 asks and verified each on the live site; it found 82 done and seven gaps. All seven are fixed here:
- **Jewelry had no Featured sort** (the page kept its own three-option select, price ascending, so it opened on the cheapest piece, an unavailable pendant). Featured, their catalogue order, is the default now, with Newest added; the same five modes as crystals and the shop.
- **A floating "Jewelry Care Guide" button** still sat under the grid above the note that links to the same guide (Sara: "it just looks a little odd that there's just a floating button"). Gone; the note's link stays.
- **The SEO folds were broken**: their descriptions had been double-escaped (six literal "&amp;amp;" on jewelry, two in the sets and craft social titles) and their sub-headings pasted as bare lines inside one paragraph. Every fold now renders headings and paragraphs, and the crystals page's fold sits in the same 760px column as the others (its `.wrap.narrow` had no rule on that page).
- **The meaning pages' Shop-all rail matched by name only**, so Black Tourmaline and Jade got nothing. Each stone's record now lists every piece whose name, first sentence or long copy names the stone (`shop` on em-stones), and the rail shows them: Black Tourmaline's bracelets, the 174Hz pieces, Tourmalinated Quartz.
- **The generator card on the intention pages had no three words** (generators live in em-frequencies, not the catalogue). The five pages load em-frequencies and print the generator's own purpose words.
- **Ten tool records still carried ALL-CAPS three-word lines**, one with a stray period. Normalised.
- **The journal's product links for stones we do not carry all went to /shop/.** Now: 50 to our product pages, three to our crystals and meaning pages, and 94 to their own collection pages on energymuse.com (Carnelian, Citrine, Smoky Quartz, the zodiac collections), which is where they pointed originally. Nothing lands on the generic shop.
Not changed, a decision for Sara: the meaning pages carry a condensed version of their pages (Heather asked for them "much simpler"; History and Origin sit behind a fold), while Sara's Loom asks that "all of the content on the current page is carried over". Their live Black Tourmaline page runs about ten thousand words. Ask her which she wants.
Also confirmed by the crawl: 60 pages and every internal link and asset on the live site return 200; all 440 product, stone and generator images load; no console errors on the homepage, frequency, quiz, product, meaning, box or With Heather pages; no horizontal overflow at 375. em-catalog v32, em-stones v5, em-journal v3.

### Sept 11: the full sweep, every remark against the live site
Clay: "full detailed sweep now.. go through everything.. is the site up to date.. ALL FEEDBACK accounted for?" Every transcript re-read a third time (whisper small.en; frames every 0.5 s for all ten 9/10 Looms: home 257, collections 1681, intention 189, nav 470, builder 507, product 271, quiz 491, blog 450, meaning 337, Heather 99), each remark checked on energy-muse-workingfolder.vercel.app after the push of a3be582. Status words: **built** (on the live site), **hers** (needs something from Sara), **answer** (a question for the reply), **call** (a decision Clay made). The transcripts themselves were then verified: all eleven recordings re-run through whisper large-v3-turbo (`~/.cache/whisper/ggml-large-v3-turbo.bin`) and diffed word by word against the small.en text; every difference was punctuation, filler or a garbled word that changed no ask ("Dory" was jewelry, "Kitson sets" kits and sets, "David Yans" the quiz option "as the day begins"). Half-second frames are enough: the change-detection sheets already drop frames that repeat.

**Homepage (2:08)** - the image she owes: hers. "Shop by Crystals feed, can we update it": answer (a catalogue feed; their collection on Shopify). Email opt-in to the footer, every page: built. The dark frequency section last: built. CTA to shop all intentions: built. "Shop by how you want to feel" pulled out: built.

**Collection pages (14:00)** - jewelry types primary, intentions secondary: built. Featured sort they dictate: built (default on shop, jewelry, crystals). Cleansing note to generators "makes no sense", a jewelry care and sizing guide instead: built (/learn/#care, her FAQ answers). Hidden SEO text per collection: built (jewelry, crystals, sets, tools; their store has none for intentions or the box). Meta tags carry over: answer (yes, on Shopify). Crystal type names theirs, "large one of a kinds" skipped: built. Crystals foot "funky, unintentional", sourcing to About: built (Beginner Guidance, Authenticity, /about/#sourcing). Frequency hero copy off the doc, "glass copper four bolts": built (three beats in her words). Sacred geometry wrong: built (her list). "One coil one designation" meaningless: built (gone). What a generator is, why you care: built (first beat and What it is). Spinning logo off this page and product pages: built. "Turn the dial is really cool" (the frames at 8:12 show her on the page's tuner, which stays), and the film's slider "also right here... repetitive, maybe keep the first two": built (the film's dial beat dropped, the tuner kept, as she suggested). Battery and specs moved up: built. Video about fixed frequency: hers (spot ready, hidden). FAQ button to the frequency group: built. Losing people who can't scroll the film: built (Skip to the generators). Kits and sets fine; box page "same SEO note": nothing to fold. At 12:48 she pauses on the sets page's "Formulated with intention. Explained step-by-step" block and both transcription models garble the sentence; the block is the copy doc's own How Sets Work module, word for word, so it stays - ask her in the reply whether she wants it changed. Tools and accessories is their Energy Tools: built (53, then the Frequency Accessories collection).

**Intention pages (1:34)** - header blurb and picture back, no CTAs: built. Three words on the cards: built. Protection's italic word: built. Generator pictures on white, same size: built. SEO copy if any: none exists.

**Nav (3:54)** - Start Here out, What's New in: built. With Heather under Learn: built. Make it yours to Personalize it: built. Personalize page copy, builder picture, Build your own bracelet with her line, build your own crystal box, Frequency Room fine, "or shop by category" with the product line: built. Images for that page: hers.

**Builders (4:13)** - "illustrative" gone: built (both). Compose for me and Fill it for me vary: built. Three words on the box tray: built. Stones not overlapped, names on them: built. Frequency Room link to the room: built. The frequency accessories feed: built (the collection, 43). A pointer to learn about the frequencies: built (under the tuner headline).

**Product pages (2:15)** - turning logo off: built. Intention eyebrow off: built. Photos in a column on desktop: built. "Goes with this, is it Shopify's AI": answer (ours now, theirs later). Founders pulled out to the bottom: built.

**Quiz (4:05)** - something you can wear, jewelry in every result, simplest picks simple, five questions: built. "A conversation about how it chooses": answer (the logic is in the quiz and HANDOFF).

**Journal (3:45)** - newest posts, Shop this post from real posts, more visual, author on every article, products higher on desktop, obvious links, personalize modules and a quiz CTA at the foot: built.

**Meaning pages (2:48)** - video lower and only when present: built. All content carried over: built (checked against their live pages; apatite has no history there either). Where to start higher, shop all the stone at the foot: built. Quiz presence high up: built. Builders at the foot: built. One more image: built (their photograph). Digital offerings feed after other stones: built.

**With Heather (0:49)** - books, deck, a featured class, a feed of the rest: built.

**Emails** - the geometry list: built. Keep the scroll-through?: call (kept, shorter, no dial). About copy in full: built. Craft modules in her order: built. Video spot that hides itself: built (products, frequency, meaning). Quiz email capture, "up to you for CRO": call (results shown, capture under the first line). Affiliates fine: nothing. Eight founders' photos: built (About, product pages).

**Sept 3 review (19 min, re-transcribed with small.en, frames every 0.5 s)** - a lifestyle hero image: hers. Shop-by-feel first: built. Frequency and the next section reversed, "not slider slider": built. Rounded buttons: built. Buttons off the photographs: built. Reviews higher: built; more than three: hers. Footer long: built (shorter, Sept 9). "Everything" to Shop all, after the categories: built. Nav dropdowns per the doc, the quiz pulled out: built (dropdowns; the quiz in the announcement bar). "Try it" to something like personalize: built. Intention hub: All last: built; "a line of text under that, probably in the copy doc": built now (the Intention Finder line, word for word). Jewelry blurb back for SEO: built (the category blurbs, Sept 7, and the folds). PDP bundling "this plus this plus this": built (Complete the practice). The long Shopify description on the PDP: built. "Does the accordion hurt SEO": answer (the text is in the page; collapsed content is indexed). Three words on PDPs and category cards: built. The black specs page merged into the frequency landing: built. Two jewelry pages (nav vs footer): built (one page, one name, one place). Jewelry types "up here in the bubble": built. Crystal types as she listed, filter by crystal: built (type pills, the stone dropdown). Kits page: Formulas and Build a Box: built. Sizing copy for Learn: built (her FAQ answers). A place for video on the meanings: built. Generator PDPs more built out, placement and accessories: built. A shoppable, dynamic product feed in the journal: built. Builders: beads and pricing from Timmi, the order handoff: hers/Shopify. Frequency Room, Formulas, systems, the black page - "too pagey", merge: built (one landing with the film, the tuner and the Formulas section; the Formulas in full on their own page, linked once).

**Clay's own notes today** - mobile add-to-bag card: built. Veza hover shade: built. Stranded "What": built. Skip link on the frequency scale: built. Raw green and black photos: built (cut-outs). Redundancy: built (twins).

### Sept 11: each product once on the shop
- The Formulas page's own copies of the Frequency Generator Bag and the Solid Copper Plate (`frequency-bag`, `copper-plate`, cat System) are the same products the tools import carries (`frequency-generator-bag`, `solid-copper-plate`). Rather than delete them from under the Formulas page, they carry `twin:"<the tool's id>"`; the shop grid, the search index and related() skip twins, and the generator product pages' accessories list points at the tools' ids. Shop all reads 218 with no name twice. The copper mini-spheres the import added were the Formulas page's `copper-spheres` under a longer name, so that import is out and `copper-spheres` carries the collection mark. `malachite-stone` and `malachite-crystal` are two products on their store and stay.
- em-search asked for `em-catalog.js?v=1` on pages that do not carry the catalogue, which a browser could have served stale for a year; it now asks for the version the page names, or the current one. em-catalog v31, em-search v12.

### Sept 11: their Frequency Accessories collection imported, and every tool photograph cut out
- **Import** (Clay: "do the import"): the 51-product Frequency Accessories collection on energymuse.com, less the seven generators (here as gen-*), the two classes (here), nine already in the catalogue, five sold out, and ten the catalogue already carried under an older id (Pyrite Cube is `wealth`, Clear Quartz Pyramid `fresh-start`, Trapiche `trapiche`, the merkaba, the selenite pyramid, the copper conductor, the four frequency rings under their long ids) - 26 new pieces: the frequency bracelets and the 7.83Hz necklace, copper harmonizers and mini-spheres, the display case, quartz and selenite pieces, the gua shas, a halite. Each carries their photographs (`assets/img/freq`, the main shot cut out, gallery shots as photographs), price, sizes where they sell sizes, first sentence, three words from their tags (two frequency bracelets carry their frequency's own name), the intention from their tags or the frequency, and their long copy in `assets/copy`. `col:["frequency-accessories"]` marks every member of the collection, old and new, 45 in all; em-catalog v30, 233 products.
- **Frequency page**: the accessories rail is the collection itself (43 for sale), eight first and a Show all; the pointer line under the tuner headline never breaks inside the link (Clay: no stranded "What"); the skip link sits in the film's first caption, not on the frequency scale (Clay's screenshot).
- **Photographs**: the 53 tools and the new mains were their raw studio shots on green and black, which is what Clay's "??" screenshots were. All 77 are subject cut-outs on transparent now, made with Apple Vision's foreground mask (`tools/cutout.swift`, thirty lines, no model download: `swiftc -O -o cutout tools/cutout.swift`, then `./cutout OUTDIR images...`), trimmed and saved as RGBA webp like the crystals. Use it for every future import; it cut a black bottle out of a black background cleanly.
- Redundancy noted, not changed: `frequency-bag` / `frequency-generator-bag` and `copper-plate` / `solid-copper-plate` exist twice (System for the Formulas page, Tool from the tools import); `malachite-stone` / `malachite-crystal`. The Formulas page and the tools page each need theirs; merging is a Shopify-side decision.

### Sept 11: the audit against all ten Looms, and the last gaps closed
Every transcript re-read line by line against the site after batch three. Closed in this round:
- **Frequency page**: the video spot moved to "Why a fixed frequency reaches further" (her Loom: the film she is making with Heather "talks about fixed frequency"), still hidden until `FREQ_VIDEO` is set; a Skip to the generators link on the film's first beat (her worry about losing people who cannot scroll past it); the grid's headline is the copy doc's "Find the frequency for you" (the film's first beat had taken "Where wellness meets tech"), with a line back to What it is for anyone arriving from Personalize it; a Frequency accessories rail from their Frequency Accessories collection, the seven pieces the catalogue carries (her assumption that the page "is going to have our entire frequency accessories category" - the other 42 of that collection, mostly frequency jewelry and copper tools, are not imported yet; on Shopify it is the collection itself).
- **Nav**: With Heather sits under Learn (her nav Loom), once; it left the phone menu's second row. em-header v55.
- **Meaning pages**: one more image block on every stone (her ask), each stone's own photograph from its live energymuse.com page (`assets/img/meaning/`), between What it is for and How to use it; How to use it now carries the live page's own opening paragraph where the page has one (seven of nine), the prototype line only for pyrite and apatite, whose pages have no such section. Checked against the live pages: apatite has no history or origin section there either, so nothing is missing from ours; angelite's history is cut down from theirs. em-stones v4.
- Checked and needing nothing: their store has no intention collections with descriptions and no build-a-box product, so there is no SEO text to fold on the intention pages or the box (she asked only "if it has SEO copy"); jewelry and crystals both default to Featured.
- Questions of hers to answer in the reply, not to build: the homepage product row is a catalogue feed (their collection on Shopify); "Goes with this" is the catalogue's related pieces, Shopify's recommendations later; the meta tags carry over on Shopify; the quiz logic is in the quiz's own comments and above.

### Sept 11: second review, batch three - About, Craft, quiz, With Heather, builders
- **About** (her email copy, in full): the hero on the founders' photograph, "Your energy is the muse"; her two opening paragraphs; Meet the founders with the then-and-now photographs; "The work has already been done for you" with Heather among the geodes; Our methodology as four rows (the title left, her paragraphs right, on phones too); the "What's next" band; her closing paragraphs and Learn with Heather. Her eight photographs are in `assets/img/life/`.
- **Craft** ("Our secret is the formulas"): her module table in her order - hero and Discover Our Method, Our methodology (the lead bold as in her doc, the three modules as rows), Handpicked and handmade, The Energy Muse ecosystem with Find Your Energy Match, Our commitment with Explore Energy Muse.
- **Quiz** (quiz Loom, her email): five questions - the copy doc's own first two (the five energies; the format, with "Something I can wear"), then where the day happens, when the practice fits, and experience. Every result carries a piece to wear (each intention page's founder favourite, name and price read from the catalogue), the stone to place, and the room's frequency; the format answer picks the lead, "simplest" leads with the least expensive (the piece when the day is spent on the move), a regular practice leads with the generator. The email capture sits under the result's first line rather than in front of it, in the doc's words ("Keep your result and receive the next step", Send My Result) - Clay's call: results shown, no gate.
- **With Heather** (Heather Loom): Learn with Heather - her two books and the card deck, one featured class, and a feed of her classes, readings and downloads. The 21 offerings and the two books are in the catalogue (cat `Class`, kept off the shop grid; books under Tools / Books) with their own product pages (no how-to panels or trio), their long copy in `assets/copy/`, photographs in `assets/img/heather/`; Crystal Muse Book shows as currently unavailable because their store lists it so. Meaning pages end with "Go deeper with Heather", four of the offerings. em-catalog v29.
- **Product pages** (product Loom): on a wide screen the photographs stack in a column under the first and the text column stays put as you scroll; the phone keeps its thumbnails. The rail skips anything out of stock. Class pages read "With Heather" and "See all classes".
- **Box builder** (builder Loom): the cells no longer overlap (they were sized from the window but laid out closer than that), each stone's name is printed under it, the three words are on the tray tiles, and "illustrative" is gone here too.
- **Intention pages** (intention Loom): the hero is two columns again - the tile's photograph and the copy doc's blurb - with the founder favourites as a full-width row under it; a Sept 9 sweep had hidden both, and the blurb now shows on phones as well.
- **Film**: the dial beat is out (Clay's call on her question about the repeat), three beats remain - Where wellness meets tech, The science behind the device, A stronger signal, a larger radius - and the stage is shorter.
- **Frequency page**: the copy doc's "Video of the process" has its spot in What it is (`#fvid`), hidden until `FREQ_VIDEO` is set in the page; `?video=vimeo:ID` or `?video=yt:ID` previews the player.
- **Homepage**: the Veza card's copper hover glow is gone (Clay: it read as a cropped brown shade on the phone); the card lifts like its neighbour.
- Meaning pages: apatite has no history on their live page either; see the audit entry above.

### Sept 11: second review, batch two - collections, journal, tools
- **Collection pages** (collection Loom): on jewelry and crystals the type row is first and primary (pills, her category names: Tumbled Stones, Shapes & Carvings, Points & Towers, Crystal Clusters; shapes and carvings share a group because their own tags do not separate them), the intention row second and underlined, All last in both; Featured (the catalogue's own order) is the default sort on crystals as it already was on the shop; their collection descriptions sit behind a "More about" fold at the foot of jewelry, crystals, sets and tools (pulled from their live pages' JSON-LD on Sept 11, `scratchpad/em-live2/collection-seo.json`); the jewelry cleansing note lost the generators button and points at a jewelry care guide on Learn built from her three FAQ care answers (`/learn/#care`); the crystals page's two floating buttons are the copy doc's Beginner Guidance and Authenticity blocks with their own calls.
- **Tools & Accessories** is their Energy Tools collection (`/energy-tools/`): 53 products pulled from energymuse.com with photographs (`assets/img/tools/`), prices, first sentences and three-word lines, in the catalogue as cat `Tool` with types Energy Tools / Home Decor / Accessories; the shop has the chip, the nav's Tools & Accessories points here (v54), em-catalog v28.
- **Journal**: the nine newest posts from their five blogs (beginners, crystals, general, guides, jewelry), newest first, with their dates, authors, featured pictures and their article HTML kept to paragraphs, headings, lists, emphasis and links; links to their products, posts and collections point at the pages here, the rest stay theirs. Bylines on articles ("By Heather Askinosie · Aug 5, 2025"); on a wide screen the article and Shop this post sit side by side, the rail sticky in a 340px column (em-journal v2).
- **Phone product card** on sets, systems and tools (Clay's screenshot): the price beside the picture-and-name row, the button full width and 44px.

### Sept 11: Sara's second review, ten Looms, batch one
Sara sent ten Looms on 9/10 (homepage, collection pages, intention pages, nav and personalize page, builders, product pages, quiz, journal, meaning pages, With Heather) and two emails (the sacred geometry list, the About and Craft copy, a video spot that hides itself, quiz email capture, eight founders' photos). Every word was transcribed (whisper small.en, 39:34 of audio) and read, with half-second frames where she pointed. Batch one, built without waiting on anyone:
- **Film geometry** matched to her list and the etchings on the devices: 174 the Vector of Equilibrium, 1111 the Vesica Piscis (on the diagonal, as on the device), 7.83 nothing.
- **Homepage:** the dark frequency section is last; the intention headline is back out above the five tiles with "Shop all intentions"; the product row's link says Shop all; the mid-page email capture is gone and the signup lives in the footer's brand block (Deanna's note, via Sara), so both footers carry it.
- **Nav:** Start Here is gone ("how it works" to the FAQ "doesn't make sense"); What's New takes its place (`/shop/?new=1`, the shop narrowed to the last 120 days, newest first); Make it yours is Personalize it everywhere (v53). The personalize page: "Personalize your energy", Build your own bracelet with her line ("choose your beads, choose your design, and we build it for you"), Build your own crystal box, the builder's own picture (a composed strand), and the Learning Center card became Shop by category.
- **Frequency page:** the spinning first-section mark hidden here (and on product pages); the copy doc's "Your space shapes you more than you think" block before the tuner with the device-in-numbers strip moved up into it; the FAQ button opens the frequency group.
- **Product pages:** the intention eyebrow above the title gone (it conflicted with the three words); the video slot on every product, rendered only when a film is in the record (`p.video` / `G.video`, `?video=` to preview); the founder line replaced by a founders block, her About paragraph and the photo of the two of them.
- **Intention pages:** the copy doc's full hero paragraph on all five; Protection's headline gets its italic word like the other four; the founder-favorites cards carry the three words; generator cards on white.
- **Builders:** "illustrative" gone; Compose for me and Fill it for me give a different result each press.
- **Journal:** articles attributed to Heather by default, a date only when the data has one; the personalize modules and a quiz call under every article.
- **Meaning pages:** a quiz line under the hero, Where to start moved up, the video only when there is one and lower down, a "Shop all {stone}" rail when the catalogue has more of that stone, the personalize modules at the foot.
Still to build from the same round: the collection rows swap and her category names, Featured sort, the hidden SEO descriptions, the cleansing note and a jewelry care guide, Tools & Accessories as their Energy Tools, the film's beats, the quiz rework and email capture, the product gallery column, the About and Craft copy with the photos, With Heather's books and classes, the journal's dates, links and side column, the box's stones with names, the crystals page's bottom block, the intention hero photo.
